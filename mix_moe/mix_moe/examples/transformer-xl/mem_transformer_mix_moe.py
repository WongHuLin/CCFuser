...
class PositionwiseFF(nn.Module):
    ...
    def forward(self, inp, attn_inp = None):
        if self.pre_lnorm:
...
from mix_moe import MixMOETransformerMLP
class CustomizedMoEPositionwiseFF(MixMOETransformerMLP):
    def __init__(self, d_model, d_inner, dropout, pre_lnorm=False, moe_num_expert=64, moe_top_k=2):
        activation = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        global world_size,expert_rank

        super().__init__(
                gate_type={}, 
                experts = {'type': 'ffn', 'num_local_expert': moe_num_expert, 
                       'hidden_size': d_inner, 'activation_fn': activation},
                model_dim=d_model,
                world_size=world_size,  
                expert_rank=expert_rank,scan_expert_func = lambda name, param: setattr(param, 'skip_allreduce', True),)

        self.pre_lnorm = pre_lnorm
        self.layer_norm = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, inp, attn_inp):
        if self.pre_lnorm:
            ##### layer normalization + positionwise feed-forward
            core_out = super().forward(self.layer_norm(inp), attn_inp)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inp, dec_attn_mask=None, mems=None):
        ...
        # self.pos_ff.async_expert_assignment(dec_inp)
        output = self.dec_attn(dec_inp, attn_mask=dec_attn_mask,
                               mems=mems)
        output = self.pos_ff(output, dec_inp)
...
class MemTransformerLM(nn.Module):
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner,
                 ...
                 world_size_ = 4, expert_rank_ = 0):
        
        global world_size,expert_rank
        world_size = world_size_
        expert_rank = expert_rank_
